...
/*   Updated: 2023/04/24 11:49:13 by oseivane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*
extract_cajon(int fd, char *cajon)
{
	char	*posit;
	size_t	bytes_read;
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	cajon = extract_cajon(fd, cajon);
	if (!cajon)
		return (NULL);
	line = extract_line(cajon);
	cajon = overwrite_cajon(cajon);
	return (line);
}
/*
int	main(void)
